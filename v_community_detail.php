...
<?php 
  ...
  $query_post = "select * from v_community where no ='$no'"; //글번호 받아와서 community 테이블에서 글찾기
      ...
      $youtubeId= $rows['youtu_id'];
      $cont = $rows['contents'];     
      $post_user_no = $rows['user_no'];                    //찾은글 row에 넣어주기 $rows[사용할쿼리];
     

  $query_reply = "select * from v_reply where post_no ='$no'"; // reply 테이블에서 댓글 찾기
  ...
  $query_user = "select * from user where num ='$post_user_no'"; // 유저닉네임 가져오기
            ...
            $user_img = "./userimg/".$user_rows['userImage'];
  ...
  <div class="container" style="max-width:1400px;width:100%;padding-top:80px;margin-bottom:135px">
    ...
    <!--사이드바 -->
      <div class="col-lg-2" style="min-width:100px;margin-top:80px;">
        <h1 class="my-4 "><a  style="color:#000066;font-weight:800;" href="v_community.php?page=1">커뮤니티</a></h1>
      ...
      <div class="col-lg-10" style="max-width:1200px;">
        ...
        <!-- 본문 -->
          <div class="card-body" style="min-width:600px;min-height:500px;padding:0px;">
             ...
             <div class="wrheader">
               <h3 class="card-title" id="title"><?php echo $rows['title'] ?>
               <span style="float:right;font-size:20px;margin-right:20px"><img src="<?php echo $user_img ?>" width=45px height=45px><?php echo $user_nickname ?></span></h3>
               <span class="writer"><span><?php echo $rows['day'] ?></span></span>
               <span class="writer ot">좋아요 <?php echo $rows['like_'] ?>  /  조회수 <?php echo $rows['view'] ?> / 댓글 <?php echo $rows['reply'] ?></span>
             </div>
            <br/>

            <div style="margin:1%auto;padding:auto; display:flex;align-items:center;justify-content:center; ">
            <div id="player"></div>
            
            </div>
            <h4 ><?php echo $rows['youtu_title']?></h4>
            <hr/>
            <div><?php echo $rows['contents']?></div>
                 ...
                 xhr.onload = function(e) {
                     if(this.status == 200) {
                         if( e.currentTarget.responseText =="1"){
                          location.replace('./v_community.php?page=1');
                           return true;
                         }else{
                           alert("다시시도해주세요");
                           return false;
